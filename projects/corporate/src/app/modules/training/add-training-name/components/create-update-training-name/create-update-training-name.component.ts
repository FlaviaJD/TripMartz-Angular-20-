...
@Component({
  selector: 'app-create-update-training-name',
  templateUrl: './create-update-training-name.component.html',
  styleUrls: ['./create-update-training-name.component.scss']
})
export class CreateUpdateTrainingNameComponent implements OnInit {
    regConfig: FormGroup;
   ...
   ngOnInit() {
    ...
    onSubmit() {
        ...
        this.subs.sink = this.apiHandlerService.apiHandler('trainingName', 'POST', {}, {}, req).subscribe(res => {
            if (res.result || res.Status) {
                this.swalService.alert.success("Training name added successfully.");
