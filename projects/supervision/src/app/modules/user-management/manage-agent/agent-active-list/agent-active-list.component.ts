...
@Component({
  selector: 'app-agent-active-list',
  templateUrl: './agent-active-list.component.html',
  styleUrls: ['./agent-active-list.component.scss']
})
export class AgentActiveListComponent implements OnInit {
    ...
    getUsersList(type){
    	...
    	this.subSunk.sink = this.apiHandlerService.apiHandler('b2cUsersList', 'post', {}, {},
            {"status": type,"auth_role_id":GlobalConstants.AGENT_AUTH_ROLE_ID})
            .subscribe(resp => {
