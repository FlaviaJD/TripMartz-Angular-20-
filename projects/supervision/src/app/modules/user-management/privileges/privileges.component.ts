...
export class PrivilegesComponent implements OnInit {
        ...
        const numberOfItems = e;
            for(let i=0;i< this.respData.length;i++) {
                if(this.respData[i].id){
                    ...
                    controlArray.controls[i].get('is_sub_privilege').setValue(this.respData[i].is_sub_privilege);
                }
            } 
        } 
    ...
    changedPrivilage(data) {
        // Assuming this.regConfig.controls.paymentDetails is a FormArray
        if (data.value && data.value.privilege_key== null || data.value.parent_key == '') {
            const paymentDetailsArray = this.regConfig.get('paymentDetails') as FormArray;
            paymentDetailsArray.controls.forEach((control, index) => {
                const controlValue = control.value;
                if (controlValue && (controlValue.parent_key === data.value.privilege_key)) {
                    paymentDetailsArray.at(index).patchValue({ isChecked: data.value.isChecked });
                }
            });
        }
            ...
            paymentDetailsArray.controls.forEach((control, index) => {
                const controlValue = control.value;
                if (controlValue && (controlValue.privilege_key === data.value.parent_key) && controlValue.parent_key=='') {
                    paymentDetailsArray.at(index).patchValue({ isChecked: data.value.isChecked });
                }
                if (controlValue && (controlValue.parent_key === data.value.privilege_key)) {
                    paymentDetailsArray.at(index).patchValue({ isChecked: data.value.isChecked });
                }
                if (controlValue && controlValue.is_sub_privilege) {
                    // If the control is a sub-privilege, set isChecked to data.value.isChecked
                    if (controlValue.parent_key === data.value.privilege_key) {
                        paymentDetailsArray.at(index).patchValue({ isChecked: data.value.isChecked });
                    }
                }
            });
