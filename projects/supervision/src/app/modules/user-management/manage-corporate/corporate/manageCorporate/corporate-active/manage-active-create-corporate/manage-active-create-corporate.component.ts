...
@Component({
  selector: 'app-manage-active-create-corporate',
  templateUrl: './manage-active-create-corporate.component.html',
  styleUrls: ['./manage-active-create-corporate.component.scss']
})
export class ManageActiveCreateCorporateComponent implements OnInit, OnDestroy {
...
paymentMode = [
  ...
  onSubmit() {
    ...
    req['auth_role_id'] =GlobalConstants.CORPORATE_AUTH_ROLE_ID;
  ...
  onPaymentModeChange(checked: Boolean, inclusion: String) {
    if (checked) {
      ...
      if (this.selectedPaymentMode.length === 0 || !this.selectedPaymentMode.includes(inclusion)) {
        ...
        // If 'credit' is selected, also add 'deposite'
        if (inclusion == 'credit') {
          this.selectedPaymentMode.push('deposite');
          this.setValue();
        }
        // If 'deposite' is selected, also add 'credit'
      ...
      // Remove the value from the selected payment modes
      this.removeValue(inclusion);
  
      // If 'credit' is deselected, remove 'deposite' as well
      if (inclusion == 'credit') {
        this.removeValue('deposite');
        this.reinitializeKey();
      }
      // If 'deposite' is deselected, remove 'credit' as well
      if (inclusion == 'deposite') {
