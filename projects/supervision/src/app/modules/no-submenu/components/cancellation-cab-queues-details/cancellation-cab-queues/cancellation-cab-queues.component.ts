...
@Component({
  selector: 'app-cancellation-cab-queues',
  templateUrl: './cancellation-cab-queues.component.html',
  styleUrls: ['./cancellation-cab-queues.component.scss']
})
export class CancellationCabQueuesComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.regConfig = this.fb.group({
            ...
            cab_status: new FormControl('CANCELLATION_PENDING', [Validators.maxLength(120)]),
    ...
    getCarQueue() {
        ...
        let reqBody = {
            ...
            "ReservationCode":this.regConfig.get('cab_app_reference').value,
            "BookedFromDate":this.regConfig.get('cab_booked_from_date').value?formatDate(this.regConfig.get('cab_booked_from_date').value, 'YYYY-MM-DD') : "",
            "BookedToDate":this.regConfig.get('cab_booked_to_date').value?formatDate(this.regConfig.get('cab_booked_to_date').value, 'YYYY-MM-DD') : "",
    ...
    applyFilter(text: string) {
        ...
        filterArray = this.respDataCopy.slice().filter((objData, index) => {
            const filterOnFields = {
                ...
                requested_Date:objData.CarDetails.CancellationDate,
                employee_reason:objData.CarDetails.CancellationFeedback,
    ...
    sortData(sort: Sort) {
        ...
        this.respData = data.sort((a, b) => {
            ...
            switch (sort.active) {
                ...
                case 'requested_Date': return this.utility.compare('' + a.CarDetails.CancellationDate, '' + b.CarDetails.CancellationDate, isAsc);
                case 'employee_reason': return this.utility.compare('' + a.CarDetails.CancellationFeedback, '' + b.CarDetails.CancellationFeedback, isAsc);
    ...
    exportExcel(): void {
        const fileToExport = this.respData.map((response: any, index: number) => {
            ...
            return {
                ...
                // "Approvar Status": response.CarDetails.BookingStatus? status :'N/A',
                "Employee Reason": response.CarDetails.CancellationFeedback? response.CarDetails.CancellationFeedback:'N/A',
