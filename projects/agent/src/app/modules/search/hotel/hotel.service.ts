...
export class HotelService implements OnDestroy {
    ...
    scrollToLocation=new BehaviorSubject<boolean>(false);
    scrollToDescription=new BehaviorSubject<boolean>(false);
    scrollToAminities=new BehaviorSubject<boolean>(false);
    ...
    private key = 'agenthoteltimer';  // The key used to store timer value in localStorage
    constructor(
    ...
    proceedWithBooking(data,created_by_id,currentUser){
        ...
        this.resetSearch();
        // data.booking_source=data.booking_source;
        //  data.CityIds[0]='111124';
        data.RequestID=0;
