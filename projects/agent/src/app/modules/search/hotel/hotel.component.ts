...
export class HotelComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this.currentUser = this.utility.readStorage('currentUser', localStorage)['user_id'];
    ...
    getSearchedList(event: any): void {
        ...
        if (event && event.target.value) {
            ...
            if (this.loggedInUser.auth_role_id === 5) {
                booking_source = 'ZBAPINO00024';
            } else if (this.loggedInUser.auth_role_id === 8) {
                const hotelApi = JSON.parse(this.loggedInUser.hotel_pricing);
