...
export class FlightApprovalListComponent implements OnInit {
  ...
  checkPolicy(){
    ...
    const selectedRecord = records.find(record => record.cabin === this.flightService.getCabinType(searchData.Segments[0].CabinClass));
   
    if(this.fareQuote.Price.TotalDisplayFare<(policyList[0].bufferAmount+selectedRecord.upper_limit)){
