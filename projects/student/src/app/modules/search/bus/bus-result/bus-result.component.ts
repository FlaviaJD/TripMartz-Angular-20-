...
getIsMobile(): boolean {
  ...
  showCancelPolicy(bus,index) {
    this.cancellationPolicyIndex=index;
  ...
  getSeatLayout(bus) {
    const created_by_id = this.util.readStorage('studentCurrentUser', localStorage)['id'];
    let request = {
  ...
  onSubmit(bus) {
    ...
    let request = {
      ...
      UserId:JSON.parse(localStorage.getItem('studentCurrentUser'))['id'],
      UserType:'B2B'
    }
