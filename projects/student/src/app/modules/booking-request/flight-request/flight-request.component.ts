...
export class FlightRequestComponent implements OnInit {
  ...
  bookNow(data){
      ...
      let currentUser = this.utility.readStorage('studentCurrentUser', localStorage);
  ...
  getVoucher(data) {
      const redirect_url = ''
      const voltureRoute = this.router.createUrlTree(['/student/reports/flight-voucher'], { queryParams: { AppReference: data.AppReference } })
