...
export class HotelPaymentDetailComponent implements OnInit, OnDestroy {
    ...
    onBooking() {
        ...
        this.submitted = true;
        let isApprovalRequired= JSON.parse(localStorage.getItem('currentUser')).approvar_required;
    ...
    ngOnDestroy() {
        const modalContainerCashFree = document.getElementById('cashfree-modal-container');
        if (modalContainerCashFree) {
            modalContainerCashFree.remove(); // forcibly closes Cashfree modal
        }
        if (this.rzp1 && typeof this.rzp1.close === 'function') {
            this.rzp1.close(); // This safely closes the Razorpay modal when component is destroyed
        }
        ...
        const razorpayContainer = document.querySelector('.razorpay-container') as HTMLElement;
        if (razorpayContainer) {
            razorpayContainer.style.display = 'none';
        }


        const modalContainerRazorPay = document.getElementById('razorpay-checkout-v2-container');
        if (modalContainerRazorPay) {
            modalContainerRazorPay.remove(); // forcibly closes Cashfree modal
        }
