...
import { ModalConfigDataI } from 'projects/employee/src/app/shared/service/mat-modal.service';
...
export class BookingComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this.renderer.listen('document', 'click', (event: MouseEvent) => {
         ...
         this.journeyListPre=JSON.parse(localStorage.getItem('b2bJourneyListPre'));
        this.segMentDate=this.journeyListPre.JourneyList.SearchData.Segments[0].DepartureDate;
    ...
    onSubmitBooking() {
        ...
        for (const passenger of passengers) {
            ...
            if (fullName.length > 68) {
                this.swalService.alert.oops(
                    "Passenger full name (incl. Title) character limit exceeded. Max 68 characters allowed."
                );
                return;
            }
            if (nameSet.has(name)) {
                this.swalService.alert.oops(
                    "Duplicate passenger name detected. Each passenger must have a unique full name."
                );
                return;
            }
            nameSet.add(name);
        ...
        for (const passenger of passengers) {
            const travellerType = passenger.travellerType;
            const today = new Date();
            let defaultDate;
            // if (!this.isPassportMandatory) {
            //     if (travellerType === "adults") {
            //         const today = new Date(this.segMentDate);
            //         const defaultDate = new Date(today);
            //         defaultDate.setFullYear(defaultDate.getFullYear() - 25);  // subtract 3 years to get DOB for age 3
            //         passenger.DateOfBirth = defaultDate;
            //     }
            //     if (travellerType === "childrens") {
            //         const today = new Date(this.segMentDate);
            //         const defaultDate = new Date(today);
            //         defaultDate.setFullYear(defaultDate.getFullYear() - 3);  // subtract 3 years to get DOB for age 3
            //         passenger.DateOfBirth = defaultDate;
            //     }
            //     if (travellerType === "infants") {
            //         const today = new Date(this.segMentDate);
            //         defaultDate.setFullYear(defaultDate.getFullYear() - 1);
            //         passenger.DateOfBirth = defaultDate;
            //     }
            // }
            const dob = new Date(passenger.DateOfBirth); // Format: "YYYY-MM-DD"
            // Calculate age in years
            let age = today.getFullYear() - dob.getFullYear();
            const m = today.getMonth() - dob.getMonth();
            const segMentDate = new Date(this.segMentDate)
            if (m < 0 || (m === 0 && segMentDate.getDate() < dob.getDate())) {
                age--;
            }
            let isValid = false;
            if (travellerType === "adults") {
                isValid = age > 18;
            } else if (travellerType === "childrens") {
                isValid = age >= 2 && age <=12;
            } else if (travellerType === "infants") {
                isValid = age >= 0 && age < 2;
            }
           if (!isValid) {
    this.swalService.alert.oops(
        `The date of birth entered does not match the expected age range for a ${travellerType}. Please correct it to proceed.`
    );
    return;
}
    ...
    commitBooking(passengersTemp,remark){
        ...
        const loggedInUser=this.utility.readStorage('currentUser', localStorage);
    ...
    addYearsToDate(y: number, m: number) {
        const d = new Date(this.segMentDate);
