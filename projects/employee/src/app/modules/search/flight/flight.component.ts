...
export class FlightComponent implements OnInit, OnDestroy {
    ...
    updateUI() {
        ...
        if (currentUrl.includes('flight/result')) {
...
this.cd.detectChanges()
    ...
    onDepart(event) {
        ...
        if (event && this.selectedIndex == 0) {
            setTimeout(() => {
                ...
                const departureDate = this.regConfig.get('departureDate').value;
                if (departureDate) {
                    this.regConfig.patchValue({ returnDate: departureDate });
                }
            }, 100)
    ...
    exchangeCityFn() {
        ...
        const departAirport = JSON.parse(JSON.stringify(this.regConfig.controls.departAirport.value));
        const destinationAirport = JSON.parse(JSON.stringify(this.regConfig.controls.destinationAirport.value));
        this.regConfig.patchValue({ departureCity: destinationCity });
        this.regConfig.patchValue({ destinationCity: departureCity });
        this.regConfig.patchValue({ departAirport: destinationAirport });
        this.regConfig.patchValue({ destinationAirport: departAirport });
    }
    ...
    onSubmit(): void {
        ...
        if (this.selectedIndex != 2 && this.regConfig.value.departureCity == this.regConfig.value.destinationCity) {
            this.showError = true;
            this.showDropdownError = false;
            return
        }
        if (this.selectedIndex == 2 && this.regConfig.value.cities) {
            for (let cities of this.regConfig.value.cities) {
                if (cities.mDepartureCity == cities.mDestinationCity) {
                    this.showError = true;
                    return
                }
            }
